...
import { SudoDefaultError } from "../models/errors/sudodefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | SudoDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | SudoDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new SudoDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new SudoDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
